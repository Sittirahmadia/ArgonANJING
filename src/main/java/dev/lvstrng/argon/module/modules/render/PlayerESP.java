...
public final class PlayerESP extends Module implements GameRenderListener {
		...
		for (PlayerEntity player : mc.world.getPlayers()) {
			if (mode.isMode(Mode.ThreeD)) {
				if (player != mc.player) {
					...
					double xPos = MathHelper.lerp(RenderTickCounter.ONE.getTickDelta(false), player.prevX, player.getX());
					double yPos = MathHelper.lerp(RenderTickCounter.ONE.getTickDelta(false), player.prevY, player.getY());
					double zPos = MathHelper.lerp(RenderTickCounter.ONE.getTickDelta(false), player.prevZ, player.getZ());
					...
					if (tracers.getValue())
						RenderUtils.renderLine(event.matrices, Utils.getMainColor(255, 1), mc.crosshairTarget.getPos(), player.getLerpedPos(RenderTickCounter.ONE.getTickDelta(false)));
	...
	private void renderOutline(PlayerEntity e, Color color, MatrixStack stack) {
		...
		Vec3d start = e.getLerpedPos(RenderTickCounter.ONE.getTickDelta(false)).subtract(camPos);
